package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1561:int = 4;
      ...
      {
         return this.var_444.length + this.var_286.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_286.length < const_1561 && this.var_444.length > 0)
            {
               _loc1_ = this.var_444.shift();
               this.var_286.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_444.indexOf(loader);
            if(index > -1)
            {
               this.var_444.splice(index,1);
            }
            index = this.var_286.indexOf(loader);
            if(index > -1)
            {
               this.var_286.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_444)
            {
               if(param2)
               {
