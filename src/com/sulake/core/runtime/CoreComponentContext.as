package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1294:String = "asset-libraries";
      
      private static const const_1290:String = "library";
      
      private static const const_1292:String = "service-libraries";
      
      private static const const_1293:String = "library";
      
      private static const const_1297:String = "component-libraries";
      
      private static const const_1298:String = "library";
      
      private static const const_1296:String = "error_data";
      
      private static const const_1295:String = "error_cat";
      
      private static const const_1291:String = "error_desc";
      ...
      private var var_339:Function;
      
      private var var_197:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1081:
               debug("Core; using simple frame update handler");
               this.var_339 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_702:
               debug("Core; using complex frame update handler");
               this.var_339 = this.complexFrameUpdateHandler;
               break;
            case Core.const_661:
               debug("Core; using profiler frame update handler");
               this.var_339 = this.profilerFrameUpdateHandler;
               this.var_197 = new Profiler(this);
               attachComponent(this.var_197,[new IIDProfiler()]);
               var_402.addChild(new ProfilerViewer(this.var_197));
               break;
            case Core.const_484:
               debug("Core; using experimental frame update handler");
               this.var_339 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_339 = this.debugFrameUpdateHandler;
         }
      }
      ...
      {
         return this.var_484 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_246[i] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1296,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1295,String(param3));
         ErrorReportStorage.setParameter(const_1291,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         try
         {
            node = config.child(const_1294)[0];
            if(node != null)
            {
         ...
         try
         {
            node = config.child(const_1297)[0];
            if(node != null)
            {
               list = node.child(const_1298);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1654);
         if(!disposed)
         {
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_711)
                  {
                     this.var_711.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_484 - this.var_340.length,this.var_484,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_197 ? 0 : this.var_1354 & 0;
         if(_loc3_ == Core.const_484)
         {
            this.var_246[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_246[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc3_:* = null;
         var _loc6_:* = null;
         var _loc4_:int = !!this.var_197 ? 0 : this.var_1354 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_105)
         {
            _loc3_ = this.var_246[_loc5_] as Array;
            if(_loc4_ == Core.const_484)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_339(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_590[priority] = 0;
            receivers = this.var_246[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_105)
         {
            ...
            {
               if(this.var_590[priority] < priority)
               {
                  ++this.var_590[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_105)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_197.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_197)
            {
               detachComponent(this.var_197);
               this.var_197.dispose();
               this.var_197 = null;
            }
            switch(this.var_1354 & 0)
            {
               case Core.const_1081:
                  this.var_339 = this.simpleFrameUpdateHandler;
                  break;
