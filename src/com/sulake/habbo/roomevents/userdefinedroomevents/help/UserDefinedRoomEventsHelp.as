package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_1125:IContainerButtonWindow;
      ...
      private function prepareWindow() : void
      {
         ...
         this._window = IFrameWindow(this.var_82.getXmlWindow("ude_help"));
         this.var_1672 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1126 = this.var_1672.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1126)
         {
            this.var_1126.procedure = this.onButtonClick;
         }
         this.var_1125 = this.var_1672.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1125)
         {
            this.var_1125.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_251 >= this.var_59.length - 1)
         {
            this.var_1125.disable();
         }
         else
         {
            this.var_1125.enable();
         }
         if(this.var_251 <= 0)
         {
            this.var_1126.disable();
         }
         else
         {
            this.var_1126.enable();
         }
      }
      ...
      {
         ++this.var_251;
         this.var_251 = Math.min(this.var_251,this.var_59.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_251;
         this.var_251 = Math.max(this.var_251,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         this.var_59 = [];
         var _loc1_:int = 1;
         while(this.var_82.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            _loc2_ = this.var_82.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title");
            _loc3_ = this.var_82.localization.getLocalization("wiredfurni.help." + _loc1_ + ".img");
            _loc4_ = this.var_82.localization.getLocalization("wiredfurni.help." + _loc1_ + ".text");
            this.var_59.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_59 || this.var_59.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_59[this.var_251][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_59[this.var_251][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_59[this.var_251][1]).value;
      ...
      {
         if(!param1 || !this.var_82 || !this.var_82.assets)
         {
            return;
         }
         var _loc2_:BitmapDataAsset = this.var_82.assets.getAssetByName(param1) as BitmapDataAsset;
         if(_loc2_ == null)
         {
      ...
      private function retrievePreviewAsset(param1:String) : void
      {
         ...
         var _loc2_:String = this.var_82.configuration.getKey("image.library.catalogue.url");
         var _loc3_:* = _loc2_ + param1 + ".gif";
         Logger.log("[ProductViewCatalogWidget] Retrieve Product Preview Asset: " + _loc3_);
         var _loc4_:URLRequest = new URLRequest(_loc3_);
         var _loc5_:AssetLoaderStruct = this.var_82.assets.loadAssetFromFile(param1,_loc4_,"image/gif");
         if(!_loc5_)
         {
            return;
         }
