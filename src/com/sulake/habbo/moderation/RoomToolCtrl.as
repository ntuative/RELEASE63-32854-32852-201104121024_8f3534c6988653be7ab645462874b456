package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private var var_1093:IDropMenuWindow;
      
      private var var_497:ITextFieldWindow;
      
      private var var_586:Boolean = true;
      
      private var var_1035:ICheckBoxWindow;
      
      private var var_1543:ICheckBoxWindow;
      
      private var var_1544:ICheckBoxWindow;
      ...
      public function show() : void
      {
         ...
         this.var_34.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_391));
         Logger.log("BEGINNING TO SHOW: " + this.var_391);
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_391);
         if(param1.flatId != this.var_391)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_391);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      public function populate() : void
      {
         ...
         this.var_497 = ITextFieldWindow(this._frame.findChildByName("message_input"));
         this.var_497.procedure = this.onInputClick;
         this.var_1093 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1093);
         this.var_1093.procedure = this.onSelectTemplate;
         this.var_1035 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
         this.var_1543 = ICheckBoxWindow(this._frame.findChildByName("lock_check"));
         this.var_1544 = ICheckBoxWindow(this._frame.findChildByName("changename_check"));
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_34.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_34,WindowTracker.const_850,this._data.flatId),this._frame,false,false,true);
      }
      ...
      {
         if(this.var_586 || this.var_497.text == "")
         {
            this.var_34.windowManager.alert("Alert","You must input a message to the user",0,this.onAlertClose);
            return;
         }
         var _loc2_:int = this.determineAction(param1,this.var_1035.isSelected);
         this.var_34.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1684,_loc2_,this.var_497.text,"",""));
         if(this.var_1543.isSelected || this.var_1544.isSelected || this.var_1035.isSelected)
         {
            this.var_34.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1543.isSelected,this.var_1544.isSelected,this.var_1035.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1695);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1537);
      }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_34.initMsg.roomMessageTemplates[this.var_1093.selection];
         if(_loc3_ != null)
         {
            this.var_586 = false;
            this.var_497.text = _loc3_;
         }
      }
   }
}
