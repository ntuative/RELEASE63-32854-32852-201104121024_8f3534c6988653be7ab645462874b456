package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2373 = new RoomThumbnailRenderer(param1);
         this.var_1157 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_179 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_179.visible = true;
         Util.hideChildren(this.var_179);
         this.refreshRoomName(this.var_179,this.var_113);
         if(this.var_113.ownerName != "" && this.var_113.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_179,!!this.var_113.event ? this.getEventCtg(this.var_113.categoryId) : this.getRoomCtg(this.var_113.categoryId));
         this.refreshRoomDesc(this.var_179,this.var_113);
         this.refreshExtraCont();
         this.refreshInfo(this.var_179,"trading_allowed",this.var_113.allowTrading);
         this.refreshInfo(this.var_179,"doormode_doorbell",this.var_113.doorMode == 1);
         this.refreshInfo(this.var_179,"doormode_password",this.var_113.doorMode == 2);
         Util.moveChildrenToColumn(this.var_179,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_179.height = Util.getLowestPoint(this.var_179);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_179,this.var_113.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_179.findChildByName("roomowner"));
         if(this.var_113.ownerName != "" && this.var_113.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_179.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1157.refreshTags(_loc1_,this.var_113.tags);
         ...
         this.var_2373.refreshThumbnail(_loc1_,this.var_113.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_254,Tabs.const_227,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
