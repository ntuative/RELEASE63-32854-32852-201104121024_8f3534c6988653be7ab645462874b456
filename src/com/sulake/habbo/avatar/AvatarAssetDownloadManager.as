package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_905:int = 3;
      
      private var var_410:AssetLoaderStruct;
      
      private var var_904:Timer;
      ...
      private const const_2031:int = 100;
      
      private const const_1561:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_410 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_904 = new Timer(this.const_2031,1);
         this.var_904.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_410)
         {
            this.var_410.addEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_410.addEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_410)
         {
            this.var_410.removeEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_410.removeEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_905);
            if(this.var_1376.indexOf("?") > 0)
            {
               _loc2_ = this.var_1376 + "&retry=" + this.var_905;
            }
            else
            {
               _loc2_ = this.var_1376 + "?retry=" + this.var_905;
            ...
            this.var_410 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2243,this._assets);
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_547[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_547[_loc3_] = _loc6_;
            }
            this.var_906[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_906)
         {
            _loc7_ = true;
            _loc4_ = this.var_906[_loc3_];
      ...
      {
         var _loc1_:* = null;
         while(this.var_734.length > 0 && this.var_735.length < this.const_1561)
         {
            _loc1_ = this.var_734[0];
            _loc1_.startDownloading();
            this.var_735.push(this.var_734.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_734.indexOf(param1) == -1 && this.var_735.indexOf(param1) == -1)
         {
