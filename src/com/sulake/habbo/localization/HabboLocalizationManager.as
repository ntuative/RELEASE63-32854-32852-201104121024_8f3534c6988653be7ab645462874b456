package com.sulake.habbo.localization
{
   ...
   public class HabboLocalizationManager extends CoreLocalizationManager implements IHabboLocalizationManager
   {
      ...
      private static var var_1338:String = "%";
      ...
      private var var_1791:Dictionary;
      ...
      {
         this.var_1791 = new Dictionary();
         this.var_2463 = ["I","II","III","IV","V","VI","VII","VIII","IX","X","XI","XII","XIII","XIV","XV","XVI","XVII","XVIII","XIX","XX"];
         super(param1,param2,param3);
         this.var_1790 = (param2 & 0) > 0;
         if(!this.var_1790)
         {
            lock();
         }
      ...
      private function fillParams(param1:String, param2:Dictionary) : String
      {
         ...
         while(_loc3_ < 10)
         {
            _loc4_ = param1.indexOf(var_1338);
            if(_loc4_ < 0)
            {
               break;
            }
            _loc5_ = param1.indexOf(var_1338,_loc4_ + 1);
            if(_loc5_ < 0)
            {
               break;
            }
      ...
      private function onHabboConfigurationInit(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         if(param2 != null)
         {
            ...
            _loc3_ = "";
            _loc3_ = this.var_190.getKey("url.prefix",_loc3_);
            _loc3_ = _loc3_.replace("http://","");
            _loc3_ = _loc3_.replace("https://","");
            super.setPredefinedUrl(_loc3_);
            if(!this.var_1790)
            {
               if("false" == this.var_190.getKey("use.default.localizations","false"))
               {
                  _loc5_ = this.var_190.getKey("external.texts.txt","external_texts.txt");
                  events.addEventListener(LocalizationEvent.const_173,this.onLocalizationLoaded);
                  super.loadLocalizationFromURL(_loc5_);
               }
            }
            _loc4_ = 1;
            while(this.var_190.keyExists("localization." + _loc4_))
            {
               _loc6_ = this.var_190.getKey("localization." + _loc4_,"");
               _loc7_ = this.var_190.getKey("localization." + _loc4_ + ".name","");
               _loc8_ = this.var_190.getKey("localization." + _loc4_ + ".url","");
      ...
      {
         var _loc2_:* = null;
         events.removeEventListener(LocalizationEvent.const_173,this.onLocalizationLoaded);
         if(this.var_190.keyExists("external.override.texts.txt"))
         {
            _loc2_ = this.var_190.getKey("external.override.texts.txt");
            events.addEventListener(LocalizationEvent.const_173,this.onLocalizationOverrideLoaded);
      ...
      {
         events.removeEventListener(LocalizationEvent.const_173,this.onLocalizationOverrideLoaded);
         unlock();
      }
      ...
      {
         this.var_1791[param1] = param2;
      }
      
      private function getBadgePointLimit(param1:String) : int
      {
         return this.var_1791[param1];
      }
      ...
      {
         return this.var_2463[Math.max(0,param1 - 1)];
      }
   }
}
