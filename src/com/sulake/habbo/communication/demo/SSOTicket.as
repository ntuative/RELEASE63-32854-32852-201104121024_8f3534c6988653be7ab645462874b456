package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      public static const const_302:String = "failure";
      ...
      private var var_1546:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         this.var_1545 = "http://" + param3 + "/client";
         if(!param4)
         {
            this.var_1546.init();
            this.var_1546.addEventListener(HabboWeb.const_247,this.requestClientURL);
         }
         else
         {
      ...
      {
         var _loc2_:String = this.var_1545;
         if(this._assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + this.var_1545);
         }
         var _loc3_:URLRequest = new URLRequest(this.var_1545);
         if(this._assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            this.var_1546.requestReAuthenticate();
         }
         else
         {
            try
            {
               this.var_1998 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               this.var_1609 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2796 = Boolean(facebookData[1]);
               }
               if(this.var_1998.length > 0)
               {
